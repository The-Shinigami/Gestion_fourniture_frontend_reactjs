...
const ChercherUtilisateur = ({childToParent}) => {
    const [ searchedValue, setValue ] = useState("1")

  const search = (searchedValue) => {
    api.get('/' + searchedValue)
      .then(response =>
      childToParent(response.data.utilisateurs));
...
export default ChercherUtilisateur;